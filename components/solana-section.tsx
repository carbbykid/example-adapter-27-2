...
const ContentSolana = () => {
  ...
  const handleSignTransaction = async () => {
    try {
      ...
      //     data: Buffer.from(
      //       'Hello, from the Coin98 Wallet Adapter example app!'
      //     ),
      //     keys: [],
  ...
  const handleSendToken = async () => {
    const kp = Keypair.fromSecretKey(
      bs58.decode(
        '37UL1qZ3yJBBpxErzUYVwXRMaL7JayBWYxpRZZPXiiBCQSGpykiaVyisw7tuKrjBc5GyP8MY852B52vyQDTLpkZE'
      )
    );
  ...
  const handleSendTokenVersionTransaction = async () => {
    ...
    setResultSendVerTrans(
      (resSend.data as string) || (resSend.error as string)
    );
  };
  ...
  return (
    <div className="grid grid-cols-1 gap-3 lg:grid-cols-2 xl:grid-cols-3">
      ...
      <CardMethod title="Send Token (Legacy)">
        <CustomButton
